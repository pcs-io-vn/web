---
title: Terms of Service
description: PCS Vietnam Terms of Service – refund policy, cancellation and privacy
---

# Terms of Service
**PCS Vietnam** · pcs.io.vn · info@pcs.io.vn  
Last updated: March 2026

---

## 1. Introduction

By subscribing to and using the services of PCS Vietnam ("PCS", "we", "us"), you ("Customer") agree to be bound by the following terms. Please read carefully before making a purchase.

---

## 2. Service Description

PCS Vietnam provides remote technical support packages for individuals, including automatic patch updates, remote support, and periodic device check-ups/optimization. Services are provided by an independent individual.

---

## 3. Subscription Plans

- **Starter Plan** – 365,000 VND/year
- **Peace of Mind Plan** – 999,000 VND/year
- **Full Care Plan** – 1,999,000 VND/year

Full feature details at: [pcs.io.vn/products](/en/products)

---

## 4. Payment Terms

- **12-month cycle** from the activation date.
- Payment is made **once** for the full cycle — **no automatic renewal**.
- Currency: VND or USD at the exchange rate at time of transaction.
- Service begins only after **email/SMS activation confirmation**.

---

## 5. Cancellation Policy

- Customers may cancel anytime via [info@pcs.io.vn](mailto:info@pcs.io.vn) or WhatsApp [+84977733339](https://wa.me/84977733339).
- Service remains active until the last day of the paid cycle.
- Subscriptions **do not auto-renew** — customers will be notified before expiry.

---

## 6. Refund Policy

- **Within 7 days** of activation: **Full refund (100%)** if no support hours have been used.
- **After 7 days**: No refund, unless the failure is on PCS's part.
- **PCS fault**: Pro-rated refund for unused time or agreed-upon alternative compensation.
- Refund requests must be submitted in writing to: [info@pcs.io.vn](mailto:info@pcs.io.vn)

---

## 7. Service Scope & Limitations

- Primary support is remote. On-site support incurs an additional **150,000 – 250,000 VND/session**.
- Unused support hours **do not roll over** to the next cycle.
- Additional hours may be purchased at **200,000 VND/hour**.
- PCS is not liable for pre-existing hardware faults or issues outside the scope of remote technical support.

---

## 8. Privacy Policy

See full policy at: [Privacy Policy](/en/privacy)

---

## 9. Support Hours

- **Monday – Friday:** After 7:00 PM
- **Saturday – Sunday:** All day
- Please book in advance via the [Service Request Form](https://calendar.app.google/r8cNH7giyokjRHP26) or [online scheduling](https://calendar.app.google/r8cNH7giyokjRHP26).

---

## 10. Changes to Terms

PCS may update these terms. Existing customers will be notified via email at least **14 days** before changes take effect. Continued use of the service after the effective date constitutes acceptance of the updated terms.

---

## 11. Contact

- 📧 [info@pcs.io.vn](mailto:info@pcs.io.vn)
- 💬 [WhatsApp: +84977733339](https://wa.me/84977733339)
- 🔗 [linktr.ee/jaxvn](https://linktr.ee/jaxvn)

---

*© 2026 PCS Vietnam. All rights reserved.*